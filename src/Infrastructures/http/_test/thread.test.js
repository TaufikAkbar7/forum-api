...
describe('/thread endpoint', () => {
    ...
    describe('when POST /threads', () => {
        ...
        it('should response 400 when request not contain needed property', async () => {
            ...
            expect(responseJson.message).toEqual('tidak dapat membuat data baru karena properti yang dibutuhkan tidak ada');
        });
        ...
        it('should response 400 when request not match type specification', async () => {
            ...
            expect(response.statusCode).toEqual(400);
            expect(responseJson.status).toEqual('fail');
            expect(responseJson.message).toEqual('tidak dapat membuat data baru karena tipe data tidak sesuai');
        });
    })
    ...
    describe('when DELETE /threads/{id}/comments/{commentId}', () => {
        it('should response 200', async () => {
            ...
            await CommentTableTestHelper.addComment({
                id: commentId,
                content: 'test comment', owner: userId, threadId
            })
        ...
        it('should response 403 when user not comment owner', async () => {
            ...
            const server = await createServer(container);
            const { accessToken, userId } = await ServerTestHelper.getAccessTokenAndUserIdHelper({ server });
            await UsersTableTestHelper.addUser({ id: 'user-444', username: 'agent', password: 'secret', fullname: 'agent super' })
            await ThreadTableTestHelper.addThread({ id: threadId, owner: userId });
            await CommentTableTestHelper.addComment({
                id: commentId,
                content: 'test comment', owner: 'user-444', threadId
            })
...
await CommentTableTestHelper.addComment({
    ...
    describe('when DELETE /threads/{id}/comments/{commentId}/replies/{replyId}', () => {
        it('should response 200', async () => {
            ...
            await CommentTableTestHelper.addComment({
                id: replyId,
                content: 'test comment', owner: userId, threadId, commentId
            })
        ...
        it('should response 403 when user not comment owner', async () => {
            ...
            await CommentTableTestHelper.addComment({
                id: replyId,
                content: 'test comment', owner: 'user-444', threadId, commentId
            })
